 ...
 *  如果程序运行中需要经常拓展数组的大小，就应该是用另一种数据结构——数据列表（array list）
 ...
 前面的数组元素的下标从0-99(而非1-100).一旦创建了数组就可以在数组中填入元素。例如使用一个循环。
        ...
        for {int i = 0; i < 100; i++}
        a[i] = i;   //a数组的元素由0-99覆盖
    创建一个数字数组时，所有元素都初始化为0.boolean数组的元素会初始化为false。对象数组的元素则初始化为一个特殊值null，表示这些元素还未存放任何对
 象。
 ...
 例如：
    ...
    有一个更简单的方式可以打印数组中的所有值，即利用Arrays类的toString方法。调用Arrays.toString(a)，返回一个包含数组元素的字符串，这些元素包
 围在中括号内，并用逗号分隔，例如，"[2, 3, 5, 7, 11, 13]"。要想打印数组，只需要调用
        System.out.println(Arrays.toString(a));

    4. 数组拷贝
    在java中允许将一个数组变量拷贝到零一个数组变量。这时，两个变量将引用同一个数组。如果希望将一个数组的所有值拷贝到一个新的数组中去，则要使用
 Arrays类的copyOf方法；
 */
