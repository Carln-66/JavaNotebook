...
/************************************
 ...
 在Java中，解决这个问题的方法是使用Java中的另外一个类，名为ArrayList。ArrayList类类似于数组，但在添加或删除元素时，
 它能够自动的调整数组容量，而不需要为此编写任何代码。
 ArrayList是一个有类型参数(type parameter)的泛型类(generic class)。为了指定数值列表保存的元素对象的类型，需要
 用一对尖括号将类名括起来追加到ArrayList后面。例如ArrayList<Employee>。
 ...
 数组列表内管理者一个内部的对象引用数组。最终，这个数组的空间有可能全部佣金；如果调用add而内部数组已经满了，
 数组列表就会自动地创建一个更大的数组，并将所有的对象从较小的数组中拷贝到较大的数组中。
 如果已经知道或能够估计出数组可能储存的元素数量，就可以在填充数组之前调用ensureCapacity方法：
 ...
 * 而容量为100个元素的数组列表，只是可能保存100个元素(实际上也可以超过100，不过要一重新分配空间为代价)，但是在最
 * 初，甚至完成初始化构造之后，数组列表不包含任何元素。

 size方法返回数组列表中包含的实际元素个数。例如
    staff.size()
 将返回staff数组列表的当前元素个数，它等价于数组a的a.length。
 一旦能够确认数组列表的大小将保持恒定，不再发生变化，就可以调用trimToSize方法。这个方法将储存块的大小调整为保存当前
 元素数量所需要的储存空间。垃圾回收器将回收多余的储存空间。
 一旦削减了数组列表的大小，添加新元素就要花时间再次移动储存快，所以应该在确认不会再向数组列表添加任何元素时再调用trimToSize.
 ...
 *  编译器会检测到这个错误。
 ...
 有时需要在数组列表的中间插入元素，为此可以使用add方法并提供一个索引参数。
    int n = staff.size() / 2;
    staff.add(n, e);
 位置n及以后的所有元素都要向后移动一个位置，为新元素留出空间。插入新元素后，如果数组列表新的大小超出了容量，数组列表就会重新
 分配他的储存数组。
 同样的，可以从数组列表中间删除一个元素：
    Employee e = staff.remove(n);
 位于这个位置之后的所有元素都向前移动一个位置，并且数组的大小减1。
 插入和删除元素的操作效率很低。对于较小的数组列表来说不需要担心这个问题，但如果储存的元素比较多，又经常需要在中间插入，删除元
 素，就应该考虑使用链表了。
 ...
 处于兼容性考虑，编译器检查到没有违反规则的现象之后，就将所有的类型化数组列表转换为初始ArrayList对象。在程序运行时，
 所有的数组列表都是一样的，即虚拟机中没有类型参数。因此，强制类型转换(ArrayList)和(ArrayList<Employee>)将执行相同的运行时
 检查。
