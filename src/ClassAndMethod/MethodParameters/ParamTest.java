package ClassAndMethod.MethodParameters;

/*******
 程序语言中如何将参数传递给方法(或函数)的一些专业术语
 * 按值调用(call by value)表示方法接受的是调用者提供的值
 * 按引用调用(call by reference)表示方法接受的是调用者提供的变量地址。方法可以修改按引用传递的变量的值，而不能修改按值
   传递的变量的值

 ”按...调用“(call by)是一个标准的计算机科学术语，用来描述各种程序语言中方法参数的传递方式

 Java程序设计语言总是采用按值调用。也就是说，方法得到的是所有参数值的一个副本。具体来讲，方法不能修改传递给它的任何参数变
 量的内容
 ...
 * 方法结束后，参数变量x不再使用。当然，对象变量harry继续引用那个工资增至3倍的员工对象
 ...
 可以看到，实现一个改变对象参数状态的方法是完全可以的，实际上也相当常见。理由很简单，方法得到的是对象引用的副本，原来的对象
 引用和这个副本都引用同一个对象。

 很多程序设计语言(尤其是C++和Pascal)提供了两种参数传递的方式：按值调用和按引用调用。实际上这种理解不对。以下反例可以说明
 这个问题。
