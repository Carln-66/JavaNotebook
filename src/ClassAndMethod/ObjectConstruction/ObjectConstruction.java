package ClassAndMethod.ObjectConstruction;
...
/*************************
 ...
 如果多个方法(比如，StringBuilder构造器方法)有相同的名字，不同的参数，便出现了重载。编译器必须跳出具体调用哪个方法。
 他用各个方法首部终得参数类型与特定方法调用中所使用的值进行匹配，来选出正确的方法。如果编译器找不到匹配的参数，就会产
 生编译时错误，因为根本不存在匹配，或者没有一个比其他的更好(这个查找匹配的过程被称为重载解析(overloading resolution))
 ...
 如果写一个类时没有编写构造器，就会为你提供一个无参数构造器。这个构造器将所有的实例字段设置为默认值。于是，实例字段终得数值型数据
 设置为0，布尔型数据设置为false，所有对象变量设置为null。
 如果类中提供了至少一个构造器，但是没有提供无参数的构造器，那么构造对象是不提供参数就是不合法的。
 例如Employee类中提供了一个简单的构造器：
    public Employee(String n, double s, int year, int month, int day)
 对于这个类，构造默认的员工就是不合法的，例如调用
    e = new Employee();     //程序报错
 * 仅当类没有其他任何构造器的时候，你才会得到一个默认的无参数的构造器。编写类的时候，如果写了一个你自己的构造器，要想让这个类的用
 * 户能够通过以下调用构造一个实例
 *      new CalssName()
 ...
 *      }
 ...
 4. 显式字段初始化
 通过重载类的构造器的方法，可以采用多种形式设置类的实例字段的初始状态。不管怎样调用构造器，每一个实例字段都要设置为一个有意义的
 初值，最好总是要确保这一点。
 可以在类定义中直接为任何字段赋值，如
    class Employee
    {
