...
图示   
![Map接口图示](https://raw.githubusercontent.com/Carln-66/img/main/Map%E6%8E%A5%E5%8F%A3%E5%9B%BE%E7%A4%BA.png)
#### 11.6.3 常用方法
...
区别在于LikedHashMap内部提供了Entry(多出了before, after用于存放前后地址)，替换HashMap中的Node。  
...
*说明：ArrayList和HashMap都是线程不安全的，如果程序要求线程安全，可以将ArrayList和HashMap转换为线程安全的。
  使用synchronizedList(List list)和synchronizedMap(Map map)*
  
## 12. 泛型
### 12.1 泛型的理解
#### 12.1.1 泛型的概念
所谓泛型，就是允许在定义类、接口时通过一个表示表示类中某个属性的类型或者是某个方法的返回值及参数类型。这个类型参数将在使用时例如，继承或实现这个接口，用这个类型声明变量、创建对象时确定)即传入实际的类型参数，也成为类型实参)。
#### 12.1.2 泛型的引入背景
...
````
图示  
![集合中没有泛型时](https://raw.githubusercontent.com/Carln-66/img/main/%E9%9B%86%E5%90%88%E4%B8%AD%E6%B2%A1%E6%9C%89%E6%B3%9B%E5%9E%8B%E6%97%B6.png)
...
图示  
![集合中使用泛型后的情况](https://raw.githubusercontent.com/Carln-66/img/main/%E9%9B%86%E5%90%88%E4%B8%AD%E6%9C%89%E6%B3%9B%E5%9E%8B%E5%90%8E%E7%9A%84%E6%83%85%E5%86%B5.png)
...
+ 在集合中使用泛型  
    ...
    + 在实例化Collection类时，可以指明具体的泛型类型  
    + 指明完以后，在集合类或接口中凡是定义类或接口时，内部结构(比如方法、构造器、属性等)使用到类的泛型的位置，都指定为实例化的泛型类型。例如add(E e) ---> 实例化以后：add(Integer e)  
    + 泛型的类型必须是类，不能是基本数据类型。需要用到基本数据类型的位置，使用包装类替换。
    + 如果没有指明泛型的类型，默认类型为java.lang.Object类型  
+ 如何自定义泛型结构：泛型类、泛型接口；泛型方法。见12.3  
...
3. 流的角色：节点流、处理流  
##### 图示
![流的分类](https://raw.githubusercontent.com/Carln-66/img/main/%E6%B5%81%E7%9A%84%E5%88%86%E7%B1%BB.png)
#### 13.2.2 流的体系结构
![IO流的体系](https://raw.githubusercontent.com/Carln-66/img/main/IO%E6%B5%81%E4%BD%93%E7%B3%BB.png)
说明：蓝框中的流需要重点关注  
...
2. 创建相应的输入流，将File类的对象作为参数传入流的构造器中
3. 具体的读入过程：  
创建相应的byte[]或char[]。  
4. 关闭流资源  
...
##### 输出过程
1. 创建File类的对象，指明写出的数据来源(不需要此文件一定要存在)
2. 创建相应的输出流，将File类的对象作为参数传入流的构造器中
3. 具体的写出过程：  
   调用write()方法：write(char[]/byte[] buffer, 0, len);
4. 关闭流资源
...
#### 13.3.2 FileInputStream / FileOutputStream
...
如果使用main()方法测试，相对路径基于当前Project的。  
Eclipse：  
...
InputStreamReader: 将与一个字节的射入六转换为字符的输入流  
解码：字节、字节数组 ---> 字符数组、字符串  
...
提供字节流和字符流之间的转换
#### 13.5.3 图示
![转换流](https://raw.githubusercontent.com/Carln-66/img/main/%E8%BD%AC%E6%8D%A2%E6%B5%81.png)
#### 13.5.4 典型实现
````
    /*
...
#### 13.6.3 数据流：DataInputStream和DataOutputStream
...
我们可以通过反射，获取对应的运行时类中的所有属性、方法、构造器、父类、接口、父类的泛型、包、注解、异常等等。
